...
$(document).on('click', '.deleteBtn', function() {
  ...
  function doDelete() {
    btn.disabled = true;
    var token = document.querySelector('meta[name="csrf-token"]') ? document.querySelector('meta[name="csrf-token"]').getAttribute('content') : (window.scope2Csrf || '');
    ...
    fetch(deleteUrl, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': token, 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } })
      ...
      .then(function(res) {
        btn.disabled = false;
        if (res.ok) {
          if (window.scope2Table && $.fn.DataTable.isDataTable('#scope2Table')) window.scope2Table.ajax.reload(null, false);
...
$(document).on('click', '.viewBtn', function() {
  var id = this.getAttribute('data-id');
  if (!id) return;
  var bodyEl = document.getElementById('scope2ViewRecordModalBody');
  var modalEl = document.getElementById('scope2ViewRecordModal');
  if (!bodyEl || !modalEl) return;
  bodyEl.innerHTML = '<div class="text-center py-5 scope2-view-loading"><div class="spinner-border text-success" role="status" style="width:2.5rem;height:2.5rem;"></div><p class="text-muted mt-3 mb-0">Loading record...</p></div>';
  ...
  fetch(fetchUrl, { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } })
    ...
    .then(function(data) {
      ...
      var html = '<div class="scope2-view-co2e"><div class="val">' + esc(co2Val) + '</div><div class="lbl">tCO₂e</div></div>';
      html += '<div class="scope2-view-details-card">';
      html += '<div class="scope2-view-row"><span class="k">Source</span><span class="v">' + esc(data.emission_source || '—') + '</span></div>';
      html += '<div class="scope2-view-row"><span class="k">Scope</span><span class="v">Scope ' + esc(String(data.scope || '—')) + '</span></div>';
      html += '<div class="scope2-view-row"><span class="k">Facility</span><span class="v">' + esc(data.facility || '—') + '</span></div>';
      html += '<div class="scope2-view-row"><span class="k">Date</span><span class="v">' + esc(dateStr) + '</span></div>';
      html += '<div class="scope2-view-row"><span class="k">Activity data</span><span class="v">' + (data.activity_data != null ? Number(data.activity_data) : '—') + '</span></div>';
      html += '<div class="scope2-view-row"><span class="k">Data source</span><span class="v">' + esc(data.data_source || '—') + '</span></div>';
      if (data.notes) html += '<div class="scope2-view-row"><span class="k">Notes</span><span class="v">' + esc(data.notes).replace(/\n/g, '<br>') + '</span></div>';
      html += '</div><div class="scope2-view-attachments"><div class="title"><i class="fas fa-paperclip me-1"></i>Attachments</div>';
      var docs = data.supporting_documents || [];
      if (docs.length === 0) html += '<p class="scope2-view-no-attach mb-0">No attachments for this record.</p>';
      else {
        for (var i = 0; i < docs.length; i++) {
          var name = (docs[i] && docs[i].split && docs[i].split('/').pop()) || ('Document ' + (i + 1));
          var url = docBase + i;
          html += '<div class="scope2-view-attachment-item"><a href="' + esc(url) + '" target="_blank" rel="noopener" title="Open in new tab">' + esc(name) + '</a><a href="' + esc(url) + '" target="_blank" rel="noopener" class="open-btn"><i class="fas fa-external-link-alt"></i> Open</a></div>';
        }
      }
      html += '</div>';
...
$(document).on('click', '.view-attachments-btn', function() {
  ...
  var body = document.getElementById('scope2AttachmentsModalBody');
  if (!body) return;
  ...
  var modalEl = document.getElementById('scope2AttachmentsModal');
